...
4. The `social-poster` skill (run on a cloud schedule) pulls this repo,
   finds the earliest `approved/<post-id>/` folder not yet in
   `posted-log.md` (plain lexicographic sort on the id — `2026-08-12-1` <
   `2026-08-12-2` < `2026-08-13-1` — so no date-parsing needed), publishes
   it to Instagram and TikTok using the raw GitHub URLs as the public image
   source, then commits a new line to `posted-log.md` and pushes — that
   commit is what prevents double-posting.

## posted-log.md format

One line per successful post, keyed by post-id (not bare date):

```
2026-08-12-1 | slide-1-affirmation.png,slide-2-scripture.png | instagram:ok tiktok:ok
2026-08-12-2 | slide-1-affirmation.png,slide-2-scripture.png | instagram:ok tiktok:ok
```

If a platform fails, it's recorded as `instagram:fail` and that post-id is
retried on the next run — only fully-succeeded post-ids are safe to skip.
